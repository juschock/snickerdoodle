...
| `private.payment_reconciliation_alerts` | deny | queue/paid-brief projections | deny | deny | payment RPC internals | no provider payload |
| rate-limit tables | deny | deny | deny | deny | named rate-limit RPCs | HMAC digests and bounded counters only |
| manager queue and access receipts | deny | queue RPC, AAL2 | deny | deny | trigger/internal sync | queue is metadata-only; access receipt records actor/count/time |
...
There are 35 `SECURITY DEFINER` routines after the SN04 migration. All 35 pin
...
Authenticated elevated surface (exactly eight routines):
