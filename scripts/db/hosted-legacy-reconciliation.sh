...
replay_clean() {
  ...
  if [[ "$count" != 23 ]]; then
    echo "Expected 23 clean migrations, replayed $count" >&2
    exit 65
  fi
}
...
echo 'SNICK_SN08A_HOSTED_RECONCILIATION_PASS postgres=17 migrations=23 paths=clean_hosted_predecessor'
